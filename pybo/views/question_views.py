...
from ..models import Question, Answer, User, question_voter, answer_voter, Category
...
def _list(category_name):
    ...
    #페이징
    question_list = question_list.filter(category.name == Question.category_name).distinct()
...
def detail(question_id):
    ...
    sub_query = db.session.query(
        answer_voter.c.answer_id, func.count('*').label('num_voter')) \
    ...
    # return render_template('question/question_detail.html', question=question, answer_list= answer._list, form=form)
...
def create(category_name):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        question = Question(subject=form.subject.data, content=form.content.data,create_date=datetime.now(), user=g.user, category_name=category.name)
        db.session.add(question)
        db.session.commit()
        return redirect(url_for('main.index', category_name=category_name))
...
def modify(question_id):
    ...
    if g.user != question.user:
        flash('수정권한이 없습니다.')
        return redirect(url_for('question.detail', question_id))
...
def delete(question_id):
    question = Question.query.get_or_404(question_id)
    if g.uesr != question.user:
        flash('삭제 권한이 없습니다')
        return redirect(url_for('question.detail', question_id=question_id))
    db.session.delete(question)
    db.session.commit()
    return redirect(url_for('question.list', category_name=question.category.name))
